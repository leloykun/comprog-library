...
template <typename T> struct segtree_vern {
    using Type = T;
    using DeltaType = T;
    const int size;
    vector<T> tree;
    const monoid<T> m;
    segtree_vern(int size, const monoid<T>& m) : size(size), tree(2 * (size + 1), m.identity), m(m) {}
    ...
    friend ostream& operator<<(ostream& os, segtree_vern& t) {
        os << "[";
